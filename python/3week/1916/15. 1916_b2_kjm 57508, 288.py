...
def dijkstra(start, end):
    ...
    # 밑에 while문 돌려면 que에 뭐가 있어야하니 마중물 격으로 1일 때 cost와 시작점(1)넣어줌
    ...
    # 그럼 heap이 필수인가? (필수는 아니나 시간 복잡도를 생각하면 합리적이다고는 할 수 있을 듯)
    # 큐에 들어있는 것들중 다음에 어느 노선을 가볼까? 고민할 때 cost가 가장 낮은걸 우선적으로 보는게 try and error 비용을 줄이니까 사용하는 것.
    # 다음 타겟팅을 할 때 매 순간 최선의 선택을 하자 (=heap을 사용하자, 굳이 cost가 큰걸 먼저 볼 필요는 없잖아?)

    # 현재 모든 disctance는 무한으로 설정 되어있는데 1자기 자신까지 가는 데 걸리는 시간은 0이니까 0으로 바꿔줨
    distance[start] = 0
    while queue:
        ...
        if distance[now] < dist:
            ...
            # (que에서 나온게 1에서 5(now)가는 비용 10(dist)이라고 하면), 기록지엔 이미 1에서 5까지 4로가는 방법을 아는데 밑에 for문 돌 필요가 없다는 로직
            continue
        ...
        # graph[1] = [(2, 2), (3, 3), (1, 4), (10, 5)] #1과 연결된 놈들 골고루 때리기 시작 (참고로 2,2에서 앞에오는 숫자가 cost임)
        for next in graph[now]:
            ...
            if cost < distance[next[1]]:
                ...
                # 오 개이득인 상황임!
                # 1에서 4가는데 왜 2를 거쳐가 바로 1에서 4로 가면 되는데
                distance[next[1]] = cost  # 2번 노드의 기록지를 더 좋은 숫자로 갈아 치운다.
                # 그리고 다른 놈들도 봐야하기 때문에 일단 가방에다가 2번 노드까지 가는데 최선의 방법을 적어서 넣는다.
                heapq.heappush(queue, (cost, next[1]))
    return distance[end]


# 왜 이걸쓰지? (__name__은 파이썬 내장 변수로 모듈의 이름을 담고 있는 문자열이고 모듈이 직접 실행되는 경우 __main__이라는 이름을 갖는대. 모듈이 다른 모듈에 의해 임포트 되어 사용될 때는 그 모듈의 이름이 들어감.)
# 코테에서는 굳이 없어도 되는 코드인듯..
