...
def dijkstra(start_point):
    ...
    while heap:
        # 첫 start_point에서 출발하여 2번째 도착점들 중 가장 비용이 낮은 곳에서 시작
        cur_wage, cur_end = heapq.heappop(heap)
        # print(graph_point)
        # 저장되어 있는 값이 현재값보다 더 적다면, 굳이 다시 체크할 필요가 없음.
        if graph_point[cur_end] < cur_wage:
            continue
        # for문을 돌면서 첫 start_point에서 갈 수 있는 모든 곳을 찍고 wage를 저장해놓고 탈출
        for new_node, new_wage in graph[cur_end]:
            wage = cur_wage + new_wage
            # print(graph_point)
            # print찍어보면 알겠지만, 처음에 1에서 5를 갈 때는 비용이 10이었으나 1에서 4를 찍고 5를 가니 비용이 4로 줄기 때문에 10 대신 4를 저장
            if graph_point[new_node] > wage:
