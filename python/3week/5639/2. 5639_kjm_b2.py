...
def postorderset(preorder, left, right):
    ...
    # 왜 +1을 해줘야하나 한참 생각했는데 for i in n하면 n 미만까지 출력하지? 그럼 for i in n-1로 하면 n-2까지만 출력됨
    ...
    # 더이상 재귀가 돌지 않을 때까지 돌고(le와 rs가 뒤바뀔때) 그때의 root를 (5임) preorder 리스트에 추가하고 root 5재귀는 스택에서 pop됨
