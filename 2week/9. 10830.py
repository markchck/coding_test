...
#실패! 했으나 결국 성공!!(행렬 제곱 계산기 완성!! 물론 분할의 개념을 쓰지 않았기에 시간 초과 뜰거고 심지어 1000으로 나누지도 않았음.)
#하지만 내힘으로 재귀를 써서 행렬 계산기를 풀었다는거에 의미가 있음.)
...
def matrix_multiple(list_l, list_r):
    ...
    # 행렬 곱할 때 아래 방식을 제일 먼저 생각했는데 이건 2제곱근까지는 되는데 3제곱근ㄱ부터는 작동하지 않음.. 2제곱한 결과에 행렬 1,2,3,4를 곱하는 과정이 생각보다 어렵거나 불가능하거나..
