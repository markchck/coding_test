...
def main():
  ...
  while min_length <= max_length:
    ...
    if count>=c: #count가 너무 많다? 설치거리인 mid_length가 너무 짧았다. 즉 min을 키워서 mid를 늘려야한다.
      # 이 코드도 똑같이 나무자르기에서 나왔던 코드인데 뭐하는 코드이냐면 count == c인 경우, 위에서 말했듯이 c개의 공유기는 설치했지만 최대는 아닐 수 있어서 한번 더 돌아야한다고 햇지? 근데 돌았는데 답이 아닐수도 있잖아? 그래서 result값을 일단 저장 시켜 놓는거다. 한번 더 돌렸다가 아예 조건이 성립하지 않았을 때 전으로 되돌릴수는 없어도 result를 미리 저장해놧으니 그걸 출력하면 되잖아?
      result = mid_length  
      min_length = mid_length + 1
    else: #count가 적다? 설치해야하는 공유기 다 설치 못했다. mid_length를 너무 넓게 잡았다. 즉 max를 늘려야한다.
      max_length = mid_length -1
   #대체 왜  mid를 출력하지 않고 result를 출력하는가? 일단 result는 무조건 count>=c를 만족하는 놈이야. 
   #근데 공유기설치 대수는 만족했지만 간격을 더 떨어지게 할 수도 있으니까 while문을 break하지 않고 더 돈단말이야?
   #만약 한번 더 돌았는데 간격을 너무 짧게 잡아서 count>=c가 else인 상황이 되어버렸다치자. 
   #그리고 min_length>max_length가 되어서 while문을 탈출하게 되었어.
   #만약 print(mid)라고 했으면 실패했을때의 mid를 추출하게 되는 셈이다. (괜히 한번 더 도전했다가 낭패를 본 상황임.)
   #근데 result = mid로 한 번 더 도박하기 전에 일단 한번 mid값을 저장하고, 도박을 하면 실패해도 result를 출력하면 되고 (mid는 result랑 다른 값이 되어있겟지)
   #만약 한 번 더 돌리는 도박이 성공하면 count>=c에 들어온다는 소리니까 result가 갱신되는거고 그걸 출력하는거다.
  print(result)
